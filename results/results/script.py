...
def preprocess_plan_file_dc(file):
...
for key in results.keys():
    ...
    for sub_key in results[key].keys():
        files = glob.glob(f'{key}/{sub_key}/output*.txt')
        if len(files) == 200:

            if 'DeepCubeA' in key:
                results[key][sub_key] = {'total_plans': len(glob.glob(f'{key}/{sub_key}/output*.txt'))}
                plan_files = glob.glob(f'{key}/{sub_key}/output*.txt')
            else:
                results[key][sub_key] = {'total_plans': len(glob.glob(f'{key}/{sub_key}/plan*.txt'))}
                plan_files = glob.glob(f'{key}/{sub_key}/plan*.txt')
            ...
            for plan_file in plan_files:
                ...
                if 'DeepCubeA' in key:
                    plan_list = preprocess_plan_file_dc(plan_file)
                else:
    ...
    for row in rows:
        if row in results[val]:
            ...
            if isinstance(data, list):
                ...
                df.at[row, val] = to_string(data).replace("total_plans", "Tot.").replace("optimal_plans", "Opt.")
        else:
            df.at[row, val] = "N/A"
