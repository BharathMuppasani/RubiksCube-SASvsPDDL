...
planner_path = '/work/bharath/rubiks_cube/scorpion1/'

problems_path = '/work/bharath/rubiks_cube/sas_problem_files/'

results_path = '/work/bharath/rubiks_cube/sas_results/'
# domain_file = '/work/bharath/rubiks_cube/domain_18action.pddl'

search_dict = {
'max-manual-pdb': 'astar(maximize([projections(manual_patterns([[ 0,  1,  2,  3]]), create_complete_transition_system=true), projections(manual_patterns([[ 4,  5,  6,  7]]), create_complete_transition_system=true), projections(manual_patterns([[ 8,  9, 10, 11]]), create_complete_transition_system=true), projections(manual_patterns([[12, 13, 14, 15]]), create_complete_transition_system=true), projections(manual_patterns([[16, 17, 18, 19]]), create_complete_transition_system=true)]))',
...
def execute_planner(plan, sas_task, output, search, planner_path):
    current_path = os.getcwd()
    os.chdir(planner_path)
    command_str = f'python fast-downward.py --overall-memory-limit 3584M --plan-file {plan} {sas_task} --search "{search}" > {output}'
    os.system(f'timeout 30m {command_str}')
    os.chdir(current_path)

def get_output(sas_task, output_file, plan_file, params_file, search_algorithm, planner_path):

    output_dict = {}
    execute_planner(plan_file, sas_task, output_file, search_dict[search_algorithm], planner_path)
...
if __name__ == '__main__':
    ...
    problem_files = glob.glob(problems_path + '/*.sas')
    for key in search_dict.keys():
        ...
        for problem_file in problem_files:
            problem_name = problem_file.replace('s3', 'problem').replace('.sas','')
            output_file = os.path.join(results_folder, problem_name.replace('problem', 'output') + '.txt')
            plan_file = os.path.join(results_folder, problem_name.replace('problem', 'plan') + '.txt')
            params_file = os.path.join(results_folder, problem_name.replace('problem', 'params') + '.json')
            get_output(problem_file, output_file, plan_file, params_file, key, planner_path)
