...
problems_path = '/work/bharath/rubiks_cube/ipc-23-SAS/dataset/sas-problems-12/'

results_path = '/work/bharath/rubiks_cube/ipc-23-SAS/results/SAS-12/'
domain_file = '/work/bharath/rubiks_cube/domain_12action.pddl'
...
def execute_planner(plan, domain, problem, output, search, planner_path):
    ...
    command_str = f'python fast-downward.py --overall-memory-limit 8192M --plan-file {plan} {problem} --search "{search}" > {output}'
...
if __name__ == '__main__':
    ...
    problem_files = sorted(glob.glob(problems_path + '*.sas'))
    for key in search_dict.keys():
        ...
        for problem_file in problem_files:
            problem_name = problem_file.split('/')[-1].replace('.sas', '')
            output_file = os.path.join(results_folder, problem_name + '-output' + '.txt')
            plan_file = os.path.join(results_folder, problem_name + '-plan' + '.txt')
            params_file = os.path.join(results_folder, problem_name + '-params' + '.json')
